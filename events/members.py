...
async def on_member_join(member: discord.Member):
    ...
    guest_channel: discord.TextChannel = get_channel(Channels.GUEST)
    await guest_channel.send(f'{member.mention} {emoji}')

    text = f"""
-Информация о гильдии Tavern of Tales - {get_channel(Channels.INFO).mention}
-Заявка для вступления в гильдию (заполняется одним сообщением прямо на канале) - {get_channel(Channels.JOIN).mention}
-Выбрать себе классовую роль для доступа к соответствующему каналу - {get_channel(Channels.CHOOSE_CLASS).mention}
-Гостевая для общения - {get_channel(Channels.GUEST).mention}
-На сервере доступна система приватных комнат - {get_channel(Channels.PRIVATE_CHANNELS).mention}
{bot.get_emoji(828026991361261619)}
"""
    embed = create_embed(description=f"Добро пожаловать в Таверну Сказаний {member.mention}!",
                         thumbnail=member.avatar_url)

    welcome = get_channel(Channels.WELCOME)
...
async def on_member_remove(member: discord.Member):
    guest_channel = get_channel(Channels.GUEST)
    guild_channel = get_channel(Channels.GUILD)
