...
class Polls(Command, name='Голосования'):
    """Команды голосования"""
    def __init__(self, bot):
        super().__init__(bot)
        self.polls: Dict[int, PollData] = {}

    def remove_poll(self, message_id) -> bool:
        try:
            self.polls.pop(message_id)
            return True
        except KeyError:
            return False
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        opposite = vote_reactions[0] if str(payload.emoji) == vote_reactions[1] else vote_reactions[1]
        channel = self.bot.get_channel(payload.channel_id)
        message_id = payload.message_id
        message: discord.Message = await channel.fetch_message(message_id)
        poll = self.polls.get(message_id)
        if all((poll, not payload.member.bot, str(payload.emoji) in vote_reactions)):
            for reaction in message.reactions:
                if str(reaction) == opposite:
